import { useListClients, useListCampaigns } from "@workspace/api-client-react";
import { Card, CardContent } from "@/components/ui/card";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Link } from "wouter";
import { Download, Plus, Mail, Phone } from "lucide-react";
...
export default function Contacts() {
  ...
  const handleExport = () => {
    ...
    const token = localStorage.getItem("ice_king_token");
    link.href = "/api/contacts/export";
    link.download = "contacts-ice-king.csv";
    // Use fetch with auth header
    fetch("/api/contacts/export", { headers: { Authorization: `Bearer ${token}` } })
      .then(r => r.blob())
      .then(blob => {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button asChild>
            <Link href="/dashboard/campaigns/new"><Plus className="h-4 w-4 mr-2" /> New Campaign</Link>
          </Button>
        </div>
      </div>
