...
export default function StampMilestonesEditor({
  ...
  return (
    <div className="space-y-4">
      <div>
        <Label className="text-base">Prizes on the card</Label>
        <p className="text-xs text-muted-foreground mt-1">
          Place a prize on any stamp (e.g. 3rd = 30% off, 5th = free ice cream, last = 50% off).
          Customers see these on their fidelity card and earn the prize when that stamp is reached.
        </p>
      </div>
      ...
      >
        <p className="text-xs font-medium text-muted-foreground mb-3 text-center">Card preview</p>
      ...
      <div className="space-y-3">
        {clamped.length === 0 && (
          <p className="text-sm text-muted-foreground text-center py-4 border border-dashed rounded-lg">
            No prizes yet. Add one for each stamp you want to reward.
          </p>
        )}
        {clamped.map((m) => (
          <div key={m.position} className="flex flex-col sm:flex-row gap-2 sm:items-end">
            ...
            <div className="flex-1">
              ...
              <Input
                className="mt-1"
                placeholder="e.g. 30% off, Free ice cream"
                value={m.label}
